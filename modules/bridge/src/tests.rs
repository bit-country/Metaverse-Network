...
use mock::{Balances, BridgeModule, RuntimeEvent, ExtBuilder, One, RuntimeOrigin, Runtime, System, Tokens};
...
fn bridge_out_nft_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(BridgeModule::bridge_out_nft(RuntimeOrigin::signed(ALICE), vec![1], (0, 1), 0));
	})
}
