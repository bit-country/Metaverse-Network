...
//!
//! The gas configurations are configurable. Right now, a pre-defined Istanbul hard fork
//! configuration option is provided.
...
pub use fp_evm::{
	Account, CallInfo, CreateInfo, ExecutionInfo, LinearCostPrecompile, Log, Precompile, PrecompileSet, Vicinity,
};
...
use frame_support::traits::{
	tokens::fungible::Inspect, Currency, ExistenceRequirement, FindAuthor, Get, Imbalance, OnUnbalanced,
	WithdrawReasons,
};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(
			...
			let info = T::Runner::create(source, init, value, gas_limit, Some(gas_price), nonce, T::config())?;
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig {
		fn build(&self) {
			for (address, account) in &self.accounts {
				...
				T::Currency::deposit_creating(&account_id, account.balance.low_u128().unique_saturated_into());
...
impl<T: Config> Pallet<T> {
	...
	pub fn account_basic(address: &H160) -> Account {
		...
		// keepalive `true` takes into account ExistentialDeposit as part of what's considered liquid
		// balance.
	...
	fn correct_and_deposit_fee(
		...
		if let Some(paid) = already_withdrawn {
			...
			let refund_imbalance =
				C::deposit_into_existing(&account_id, refund_amount).unwrap_or_else(|_| C::PositiveImbalance::zero());
