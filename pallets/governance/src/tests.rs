...
fn get_next_proposal_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let hash = set_balance_proposal_hash(0);
		let hash2 = set_balance_proposal_hash(1);
		let hash3 = set_balance_proposal_hash(2);
		let hash4 = set_balance_proposal_hash(3);
		let hash5 = set_balance_proposal_hash(4);
		add_preimage(hash);
		add_preimage(hash2);
		add_preimage(hash3);
		add_preimage(hash4);
		...
		assert_eq!(GovernanceModule::deposit_of(0),Some((vec![1], 500)));
		assert_ok!(GovernanceModule::propose(
			origin.clone(),
			ALICE_COUNTRY_ID,
			600,
		...
		assert_eq!(GovernanceModule::deposit_of(1),Some((vec![1], 600)));
		assert_ok!(GovernanceModule::propose(
			origin.clone(),
			ALICE_COUNTRY_ID,
			700,
		...
		assert_eq!(GovernanceModule::deposit_of(2),Some((vec![1], 700)));
