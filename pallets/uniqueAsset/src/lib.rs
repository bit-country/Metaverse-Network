...
pub trait Trait: system::Config + pallet_balances::Config {
    /// The Asset ID type
...
pub type CollectionInfoOf<T> =
    CollectionInfo<<T as system::Config>::AccountId, <T as Trait>::CollectionData>;
pub type AssetInfoOf<T> = AssetInfo<<T as system::Config>::AccountId, <T as Trait>::AssetData>;
