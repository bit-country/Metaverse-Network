...
pub mod pallet {
	...
	pub type LandByOwner<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, Vec<LandId>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10_000)]
		pub fn transfer_land(origin: OriginFor<T>, to: T::AccountId, land_id: LandId) -> DispatchResultWithPostInfo {
			let who = ensure_signed(origin)?;
			/// Get owner of the land
			LandOwner::<T>::try_mutate_exists(&land_id, &who, |land_by_owner| -> DispatchResultWithPostInfo {
...
impl<T: Config> Module<T> {
	...
	fn get_new_land_id() -> Result<LandId, DispatchError> {
		let land_id = NextLandId::<T>::try_mutate(|id| -> Result<LandId, DispatchError> {
			let current_id = *id;
			*id = id.checked_add(One::one()).ok_or(Error::<T>::NoAvailableLandId)?;
			Ok(current_id)
		})?;
		Ok(land_id)
	}
