...
pub mod pallet {
    ...
    pub type ContinuumSpots<T: Config> =
        StorageMap<_, Twox64Concat, SpotId, ContinuumSpot, OptionQuery>;

    /// Continuum Spot Position
    #[pallet::storage]
    #[pallet::getter(fn get_continuum_position)]
    pub type ContinuumCoordinates<T: Config> =
        StorageMap<_, Twox64Concat, (i32, i32), SpotId, OptionQuery>;

    /// Active Auction Slots of current session index that accepting participants
    #[pallet::storage]
    #[pallet::getter(fn get_active_auction_slots)]
    pub type ActiveAuctionSlots<T: Config> = StorageMap<
        _,
        Twox64Concat,
        T::BlockNumber,
        Vec<AuctionSlot<T::BlockNumber, T::AccountId>>,
        OptionQuery,
    >;
    ...
    pub type GNPSlots<T: Config> = StorageMap<
        _,
        Twox64Concat,
        T::BlockNumber,
        Vec<AuctionSlot<T::BlockNumber, T::AccountId>>,
    ...
    pub type EOISlots<T: Config> =
        StorageMap<_, Twox64Concat, T::BlockNumber, Vec<SpotEOI<T::AccountId>>, ValueQuery>;
    ...
    pub type ReferendumInfoOf<T: Config> = StorageMap<
        ...
        Twox64Concat,
        SpotId,
        ReferendumInfo<T::AccountId, T::BlockNumber>,
        OptionQuery,
    >;
    ...
    pub type VotingOf<T: Config> =
        StorageMap<_, Twox64Concat, T::AccountId, Voting<T::AccountId>, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn buy_continuum_spot(
            ...
            let sender = ensure_signed(origin)?;
                ...
                AllowBuyNow::<T>::get() == true,
                Error::<T>::ContinuumBuyNowIsDisabled
            );
        ...
        pub fn register_interest(
            ...
            if EOISlots::<T>::contains_key(current_active_session_id) {
                ...
                EOISlots::<T>::try_mutate(
                    current_active_session_id,
                    |spot_eoi| -> DispatchResult {
                        /// Check if the interested Spot exists
                        let interested_spot_index: Option<usize> =
                            spot_eoi.iter().position(|x| x.spot_id == spot_id);
                        match interested_spot_index {
                            /// Already got participants
                            Some(index) => {
                                /// Works on existing eoi index
                                let interested_spot =
                                    spot_eoi.get_mut(index).ok_or("No Spot EOI exist")?;
        ...
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
        pub fn enable_bidder_rejection_voting(
            origin: OriginFor<T>,
            spot_id: SpotId,
        ) -> DispatchResultWithPostInfo {
        ...
        pub fn vote(
            origin: OriginFor<T>,
            id: SpotId,
            reject: AccountVote<T::AccountId>,
        ) -> DispatchResultWithPostInfo {
            let sender = ensure_signed(origin)?;
        ...
        pub fn emergency_shutdown(
            ...
            T::EmergencyOrigin::ensure_origin(origin)?;
                ...
                !Cancellations::<T>::contains_key(spot_id),
...
impl<T: Config> Pallet<T> {
    ...
    fn referendum_info(
        spot_id: SpotId,
    ) -> Result<ReferendumInfo<T::AccountId, T::BlockNumber>, DispatchError> {
        let info =
            ReferendumInfoOf::<T>::get(spot_id).ok_or(Error::<T>::ReferendumIsInValid.into());
        info
    }
    ...
    pub fn check_spot_ownership(
        spot_id: Option<SpotId>,
        coordinate: (i32, i32),
    ) -> Result<SpotId, DispatchError> {
        match spot_id {
            None => {
                ...
                let next_spot_id =
                    NextContinuumSpotId::<T>::try_mutate(|id| -> Result<SpotId, DispatchError> {
                        let current_id = *id;
                        *id = id
                            .checked_add(One::one())
                            .ok_or(Error::<T>::SpotIsNotAvailable)?;
