...
use primitives::{AssetId, BlockNumber, ClassId, GroupCollectionId, Hash, ItemId, TokenId, NftMetadata, Attributes, LAND_CLASS_ID, ESTATE_CLASS_ID};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn mint(
			...
			let minting_result = Self::do_mint_nfts(&sender, class_id, metadata, attributes, quantity)?;
			...
			Self::deposit_event(Event::<T>::NewNftMinted(
				*minting_result.0.first().unwrap(),
				*minting_result.0.last().unwrap(),
				sender,
				class_id,
				quantity,
				minting_result.1,
...
impl<T: Config> NFTTrait<T::AccountId> for Pallet<T> {
	...
	fn get_nft_detail(asset_id: (Self::ClassId, Self::TokenId)) -> Result<(GroupCollectionId, Self::ClassId, Self::TokenId), DispatchError> {
