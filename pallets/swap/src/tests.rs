...
fn add_liquidity_should_work_with_greater_max_native_amount() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::swap(crate::Event::AddLiquidity(
            ALICE, NUUM_SOC.0, 10, NUUM_SOC.1, 5, 20,
        ));
...
fn add_liquidity_should_work_with_greater_max_social_amount() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::swap(crate::Event::AddLiquidity(
            ALICE, NUUM_SOC.0, 5, NUUM_SOC.1, 10, 20,
        ));
        assert_eq!(last_event(), event);
    });
}
...
fn add_liquidity_should_work_when_share_issuance_greater_than_zero() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::swap(crate::Event::AddLiquidity(
            ALICE, NUUM_SOC.0, 5, NUUM_SOC.1, 5, 10,
        ));
        assert_eq!(last_event(), event);
...
fn remove_liquidity_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::swap(crate::Event::RemoveLiquidity(
            ALICE, NUUM_SOC.0, 2, NUUM_SOC.1, 1, 5,
        ));
