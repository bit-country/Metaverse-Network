...
benchmarks! {
    ...
    add_minting_origin {
		let origin: T::AccountId = whitelisted_caller();
		let who: T::AccountId = account("target", 0, SEED);
	}: _(RawOrigin::Signed(origin.clone()), who.clone()) 
	verify {
		assert_eq!(crate::Pallet::<T>::ensure_admin(RawOrigin::Signed(origin.clone()).into()), Ok(()));
		assert_eq!(crate::Pallet::<T>::is_mining_origin(&origin.clone()), true);
	}
    ...
    remove_minting_origin {
		...
		crate::Pallet::<T>::add_minting_origin(RawOrigin::Signed(origin.clone()).into(), who.clone());
	}: _(RawOrigin::Signed(origin.clone()), who.clone()) 
	verify {
		assert_eq!(crate::Pallet::<T>::ensure_admin(RawOrigin::Signed(origin.clone()).into()), Ok(()));
		assert_eq!(crate::Pallet::<T>::is_mining_origin(&origin.clone()), false);
	}
	...
	update_mining_issuance_config {
		let origin: T::AccountId = whitelisted_caller();
	}: _(RawOrigin::Signed(origin.clone()), MINING_RESOURCE_RATE_INFO)
	verify {
		assert_eq!(crate::Pallet::<T>::ensure_admin(RawOrigin::Signed(origin.clone()).into()), Ok(()));
