...
fn mint_mining_resource_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::mining(
            crate::Event::MiningResourceMinted(1000)
        );
...
fn burn_mining_resource_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(last_event(), mock::Event::mining(
            crate::Event::MiningResourceBurned(300)
        ));
    });
}
...
fn withdraw_mining_resource_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let event = mock::Event::mining(
            crate::Event::WithdrawMiningResource(BOB, 300)
        );
...
fn deposit_mining_resource_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        //Transfer to BOB 300
        assert_ok!(Currencies::transfer(Origin::signed(treasury_id), BOB, MiningCurrencyId::get(), 300));
        ...
        let event = mock::Event::mining(
            crate::Event::DepositMiningResource(BOB, 100)
        );
