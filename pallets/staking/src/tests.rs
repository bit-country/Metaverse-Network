...
use crate::mock::{
	events, last_event, roll_to, set_author, Balances, Event as MetaEvent, ExtBuilder, Origin, Stake, Test,
};
...
fn set_total_selected_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(last_event(), MetaEvent::Stake(Event::TotalSelectedSet(5u32, 6u32,)));
	});
}
...
fn set_parachain_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(last_event(), MetaEvent::Stake(Event::ParachainBondAccountSet(0, 11)));
	});
}
...
fn leave_candidates_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(last_event(), MetaEvent::Stake(Event::CollatorScheduledExit(1, 1, 3)));
		});
}
...
fn go_offline_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(last_event(), MetaEvent::Stake(Event::CollatorWentOffline(1, 1)));
		});
}
...
fn go_online_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(last_event(), MetaEvent::Stake(Event::CollatorBackOnline(1, 1)));
		});
}
...
fn candidate_bond_more_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(last_event(), MetaEvent::Stake(Event::CollatorBondedMore(1, 20, 50)));
		});
}
...
fn candidate_bond_less_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(last_event(), MetaEvent::Stake(Event::CollatorBondedLess(1, 30, 20)));
		});
}
...
fn leave_nominators_event_emits_correctly() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			roll_to(1);
			assert_ok!(Stake::leave_nominators(Origin::signed(2), 1));
			assert_eq!(last_event(), MetaEvent::Stake(Event::NominatorExitScheduled(1, 2, 3)));
			roll_to(10);
...
fn leave_nominators_removes_nominations_from_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			roll_to(1);
			for i in 2..6 {
				let candidate_state = Stake::collator_state2(i).expect("initialized in ext builder");
...
fn nominator_bond_less_updates_just_bottom_nominations() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let pre_call_collator_state = Stake::collator_state2(&1).expect("nominated by all so exists");
			assert_ok!(Stake::nominator_bond_less(Origin::signed(2), 1, 2));
			let post_call_collator_state = Stake::collator_state2(&1).expect("nominated by all so exists");
...
fn nominator_schedule_revocation_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Stake::revoke_nomination(Origin::signed(2), 1));
			assert_eq!(
				Stake::nominator_state2(2).expect("exists").scheduled_revocations_total,
				10
			);
			roll_to(10);
			assert_eq!(
...
fn parachain_bond_inflation_reserve_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// keep paying 6
			let mut new3 = vec![
				Event::ParachainBondReservePercentSet(Percent::from_percent(30), Percent::from_percent(50)),
