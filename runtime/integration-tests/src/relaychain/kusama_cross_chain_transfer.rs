...
fn transfer_to_sibling() {
	...
	Sibling::execute_with(|| {
		assert_eq!(Tokens::free_balance(BNC, &neer_reserve_account()), 90_000_000_000_000);
		...
		assert_eq!(Tokens::free_balance(BNC, &neer_reserve_account()), 95_000_000_000_000);
...
fn transfer_from_relay_chain_deposit_to_treasury_if_below_ed() {
	...
	Pioneer::execute_with(|| {
		...
		assert_eq!(
			Tokens::free_balance(KSM, &pioneer_runtime::TreasuryModuleAccount::get()),
			1_000_128_000_111
		);
	});
}
...
fn trap_assets_lower_than_ed_works() {
	...
	Pioneer::execute_with(|| {
		assert_ok!(Tokens::deposit(
			...
			dollar(RelayChainCurrencyId)
		));
		let _ =
			pallet_balances::Pallet::<Runtime>::deposit_creating(&AccountId::from(DEFAULT), dollar(NativeCurrencyId));
		neer_treasury_amount = Currencies::free_balance(NativeCurrencyId, &TreasuryModuleAccount::get());
	});
	...
	KusamaNet::execute_with(|| {
		let xcm = vec![
			...
			BuyExecution {
				fees: assets,
				weight_limit: Limited(dollar(RelayChainCurrencyId) as u64),
			},
			WithdrawAsset(
				(
					(Parent, X2(Parachain(2000), GeneralKey(NativeCurrencyId.encode()))),
					kar_asset_amount,
				)
					.into(),
			),
	...
	Pioneer::execute_with(|| {
		...
		assert_eq!(
			ksm_asset_amount + dollar(RelayChainCurrencyId),
			Currencies::free_balance(RelayChainCurrencyId, &TreasuryModuleAccount::get())
		);
		assert_eq!(
			kar_asset_amount,
			Currencies::free_balance(NativeCurrencyId, &TreasuryModuleAccount::get()) - neer_treasury_amount
		);
	});
}
...
fn sibling_trap_assets_works() {
	...
	Pioneer::execute_with(|| {
		assert_ok!(Tokens::deposit(BNC, &sibling_account(), dollar(ParachainCurrencyId)));
		let _ = pallet_balances::Pallet::<Runtime>::deposit_creating(&sibling_account(), dollar(NativeCurrencyId));
		neer_treasury_amount = Currencies::free_balance(NativeCurrencyId, &TreasuryModuleAccount::get());
	});
	...
	Pioneer::execute_with(|| {
		...
		assert_eq!(
			Currencies::free_balance(ParachainCurrencyId, &TreasuryModuleAccount::get()),
			kar_asset_amount
		);
	});
}
