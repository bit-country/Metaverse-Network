...
fn transfer_to_sibling() {
	...
	Pioneer::execute_with(|| {
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(ALICE)), 100_000_000_000_000);
		...
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(ALICE)), 90_000_000_000_000);
	});

	Sibling::execute_with(|| {
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &neer_reserve_account()), 100_000_000_000_000);
		// assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(BOB)), 10_000_000_000_000);
		assert_eq!(
			Tokens::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(BOB)),
			10_000_000_000_000
		);
		...
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &neer_reserve_account()), 95_000_000_000_000);
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(BOB)), 4_989_760_000_000);
	});

	Pioneer::execute_with(|| {
		assert_eq!(Currencies::free_balance(PARA_CHAIN_CURRENCY, &AccountId::from(ALICE)), 94_989_760_000_000);
	});
}
...
fn transfer_from_relay_chain_deposit_to_treasury_if_below_ed() {
	...
	Pioneer::execute_with(|| {
		...
		assert_eq!(
			Currencies::free_balance(RELAY_CHAIN_CURRENCY, &pioneer_runtime::TreasuryModuleAccount::get()),
			1_000_128_000_111
		);
	});
}
...
ParachainInfo>::utility_as_derivative_call(transfer_call, 0); 		let weight = 10_000_000_000;
	...
	KusamaNet::execute_with(|| {
		...
		assert_eq!(
			kusama_runtime::Balances::free_balance(&sub_account),
			1000 * dollar(RELAY_CHAIN_CURRENCY_ID)
		);
		assert_eq!(
			kusama_runtime::Balances::free_balance(&AccountId::from(BOB)),
			dollar(RELAY_CHAIN_CURRENCY_ID)
...
fn trap_assets_larger_than_ed_works() {
	...
	let (ksm_asset_amount, kar_asset_amount) = (dollar(RELAY_CHAIN_CURRENCY_ID), dollar(PARA_CHAIN_CURRENCY_ID));
	let trader_weight_to_treasury: u128 = 96_000_000;

	Pioneer::execute_with(|| {
		assert_ok!(Tokens::deposit(RELAY_CHAIN_CURRENCY, &AccountId::from(DEFAULT), 100 * dollar(RELAY_CHAIN_CURRENCY_ID)));
		let _ = pallet_balances::Pallet::<Runtime>::deposit_creating(&AccountId::from(DEFAULT), 100 *
dollar(PARA_CHAIN_CURRENCY_ID));

		neer_treasury_amount = Currencies::free_balance(PARA_CHAIN_CURRENCY, &TreasuryModuleAccount::get());
	});
	...
	KusamaNet::execute_with(|| {
		let xcm = vec![
			...
			WithdrawAsset(
				(
					(Parent, X2(Parachain(2000), GeneralKey(PARA_CHAIN_CURRENCY_ID.encode()))),
					kar_asset_amount,
				)
					.into(),
			),
	...
	Pioneer::execute_with(|| {
		...
		assert_eq!(
			trader_weight_to_treasury + dollar(RELAY_CHAIN_CURRENCY_ID),
...
fn sibling_trap_assets_works() {
	...
	Pioneer::execute_with(|| {
		assert_ok!(Tokens::deposit(PARA_CHAIN_CURRENCY, &sibling_account(), dollar(PARA_CHAIN_CURRENCY_ID)));
